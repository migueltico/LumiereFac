...
$("#bodyContent").on("keypress", "#searchProductOfertaEdit", function (e) {
	if (e.charCode == 13) {
		if (e.target.value.length > 0) {
			searchProductToOferta(
				e,
				"searchProductOfertaEdit",
				"ofertaRowsEditModal"
			);
		} else {
			Swal.fire({
	...
	fetch("/admin/ofertas/getofertasbyid", {
		...
		.then((resp) => {
			if (resp.estado && resp.error == "00000") {
				...
				let idbtnEdit = document.getElementById(
					"ofertas_add_btnEditOferta"
				);
				let nombre = document.getElementById("ofertas_edit_nombre");
				let cantidad = document.getElementById(
					"ofertas_edit_cantidad"
				);
				let tipoOferta = document.getElementById(
					"ofertas_edit_tipooferta"
				);
				let descuento = document.getElementById(
					"ofertas_edit_descuento"
				);
...
$("#bodyContent").on("click", "#ofertas_add_btnAddOferta", function (e) {
	let formData = new FormData(
		document.getElementById("ofertas_AddOfertaForm")
	);
...
$("#bodyContent").on("click", "#ofertas_add_btnEditOferta", function (e) {
	let formData = new FormData(
		document.getElementById("ofertas_EditOfertaForm")
	);
	let idbtnEdit = document.getElementById("ofertas_add_btnEditOferta");
...
function validateInputCantRow(input) {
	...
	if (max >= value) {
		...
		Swal.fire({
			...
			text: `La cantidad de ${input.value} Unidad${
				input.value > 1 ? "es" : ""
			} supera la cantidad de ${input.max} Unidad${
				input.max > 1 ? "es" : ""
			} facturada${input.max > 1 ? "s" : ""} del producto.`,
			icon: "error",
			confirmButtonText: "OK",
			// timer: 8000,
			// timerProgressBar: true
...
function calcAmountAllRowDevolution() {
	...
	let saldoActual_devolucion = document.getElementById(
		"saldoActual_devolucion"
	).innerHTML;
	let newTotal = 0.0;
	for (const child of childs) {
		if (child.children[8].children[0].checked) {
	...
	if (resultRegex) {
		...
		fetch("facturacion/consultar/factura", {
			...
			.then((resp) => {
				if (resp !== null) {
					let idfacDevolucion =
						document.getElementById("idfacDevolucion");
					let fecha_max_devolucion = document.getElementById(
						"fecha_max_devolucion"
					);
					let fecha_venta_devolucion = document.getElementById(
						"fecha_venta_devolucion"
					);
					let saldoActual_devolucion = document.getElementById(
						"saldoActual_devolucion"
					);

					saldoActual_devolucion.innerText = resp.hasDevolution
						? resp.devolucion_details.Saldo
						: "0.00";
					...
					resp.details.forEach((element, index) => {
						...
						if (
							element.originalCant != undefined &&
                ...
                <tr id="ChangesRow_${element.idproducto}">
                    ...
                    <td data-realcant="${element.cantidad}">${
							element.originalCant
						} ${cantidadModificada}</td>
                    <td>${element.precio}</td>
                    <td class='text-center'>${element.descuento}%</td>
                    <td class='text-center'>${element.iva}%</td>
                    <td>${element.total}</td>
                    <td><input class="inputCantFactRow" style='text-align:center' ${
					element.cantidad == 0 ? "disabled" : ""
				} type="number" min='${
							element.cantidad > 0 ? 1 : 0
						}' max='${element.cantidad}' value='${
							element.cantidad > 0 ? 1 : 0
						}'></td>
                    <td class='text-center'><input class="inputSelectRowFact" type='checkbox' ${
					element.cantidad == 0 ? "disabled" : ""
				} data-codigo="${element.idproducto}" data-cantidad="${
							element.cantidad
						}" data-precio="${
							element.precio
						}" data-descuento="${
							element.descuento
						}" data-iva="${element.iva}" data-total="${
							element.total
						}" ></td>
...
$("#bodyContent").on("click", ".btnDeleteRowTarjeta", function (e) {
	...
	let totalFactAmount =
		document.getElementById("totalFactAmount").textContent;
	let amountsaldo =
		document.getElementById("totalFactAmount").dataset.amountsaldo;
	let row = document.getElementById(id);
	row.remove();
	if (!cb.checked) {
...
$("#bodyContent").on("click", "#addNewTarjeta", function (e) {
	...
	let letras = ["a", "b", "c", "d"];
	let randID =
		letras[Math.floor(Math.random() * 4)] +
	...
	input2.classList.add(
		"form-control",
		"tarjertaInputs_monto",
		"inputRowsTarjetaMonto",
		"lbMontoToPay"
	);
	input2.value = "0.00";
...
function multiState(e) {
	...
	let pagoContraEntrega =
		document.getElementById("pagoContraEntrega").checked;
	if (!pagoContraEntrega) {
		if (MultipagoSwitch.checked) {
			if (hasClass(btnApartado, "active")) {
				...
				$(".lbMontoToPay").val(
					roundUpToNearest100(
						totalFactura.dataset.amount.replace(",", "") / 3
					).toFixed(2)
				);
			...
			if (hasClass(btnApartado, "active")) {
				...
				if (
					...
					$(".lbMontoToPay").val(
						totalFactura.dataset.amountsaldo
					);
				} else {
					$(".lbMontoToPay").val(
						roundUpToNearest100(
							totalFactura.dataset.amount.replace(
								",",
								""
							) / 3
						).toFixed(2)
				...
				$(".cardHeaderSwitch")
					.first()
					.addClass("selectedMethodPay");
				$(".fact_switchBtns input[type='checkbox").prop(
					"checked",
					false
				);
...
$("#bodyContent").on("click", "#PrintFactBtn", function (e) {
	...
	if (hasClass(btnApartado, "active")) {
		$(".lbMontoToPay").prop("disabled", false);
		$(".lbMontoToPay").val(
			roundUpToNearest100(amountFloat / 3).toFixed(2)
		);
	} else {
		$(".lbMontoToPay").prop("disabled", true);
	}
...
$("#bodyContent").on("click", "#group_type_fac .btn", function (e) {
	...
	if (
		parseInt(cliente.dataset.idgenerico) ==
		parseInt(cliente.dataset.cliente)
	) {
		if (abono || envio) {
...
$("#bodyContent").on("click", "#btnMakeFact", function (e) {
	...
	if (fac_active_btn == 1) {
		alert(
			"Ya se encuentra una factura en proceso, por favor evite el doble click al facturar"
		);
		return;
	} else {
...
function getProductsRowsForFac(
	method,
	pagoContraEntrega,
	isApartado,
	tipoPago
	...
	let idVendedor =
		document.getElementById("InputVendedorFact").dataset.vendedor;
	...
	let itemsFac = [];
	let gran_subtotal = 0.0;
	let gran_subtotal_descuento = 0.0;
	...
	for (let row of rows) {
		...
		let iva =
			row.children[3].textContent > 0
				...
				: 0;
		let precio = ConvertLabelFormat(
			parseFloat(row.children[6].textContent)
		);
		let subtotal = ConvertLabelFormat(
			parseFloat(row.children[8].textContent)
		);
		let total_iva = ConvertLabelFormat(
		...
		gran_subtotal +=
			parseFloat(row.children[6].textContent) * parseInt(cantidad);
		itemsFac.push(json);
	}
	...
	let inputSearchFacSaldo = document.getElementById(
		"inputSearchFacSaldo"
	).value;
	let inputFacNewSaldoNow = document.getElementById(
		"inputFacNewSaldoNow"
	).value;
	let finalJson = {
		items: itemsFac,
		...
		descuento: ConvertLabelFormat(
			gran_subtotal - gran_subtotal_descuento
		), // precio sin descuento menos precio con descuento
		iva,
		idCliente: idCliente,
		...
		hasSaldo:
			localStorage.saldo != "false" && localStorage.saldo != undefined
				? true
				: false,
		saldo:
			localStorage.saldo != "false" && localStorage.saldo != undefined
				? parseFloat(saldoUsed)
				: false,
	...
	setTimeout(() => {
		//Temporizador para evitar doble factura
		printFact(finalJson);
	}, 700);
}
...
function PagoMultipleFac(hasPagoNormal, isApartado) {
	...
	let montoTotalFactura =
		document.getElementById("totalFactAmount").dataset.amount;
	let Allswitch = document.getElementsByClassName("switchGroupAmount");
	let sumaDeCamposTotal = 0.0;
	let result;
	let methodsArray = [];
	for (let switchItem of Allswitch) {
		if (switchItem.checked) {
			...
			if (result.state) {
				let monto = document.getElementsByClassName(
					`${itemId}_monto`
				)[0].value;
				...
				sumaDeCamposTotal =
					parseFloat(sumaDeCamposTotal) + parseFloat(valor);
	...
	if (
		(parseFloat(sumaDeCamposTotal) == parseFloat(montoTotalFactura) &&
			hasPagoNormal) ||
...
function verificaCamposPago(inputClass, multi) {
	switch (inputClass) {
		case "efectivoInputs":
			let Emonto = document.getElementsByClassName(
				`${inputClass}_monto`
			)[0].value;
		...
		case "tarjertaInputs":
			let pagoContraEntrega =
				document.getElementById("pagoContraEntrega").checked;
			let abono = btnTypeApartado.classList.contains("active");
			let Tmonto = document.getElementsByClassName(
				`${inputClass}_monto`
			)[0].value;
			let tarjeta = document.getElementsByClassName(
				`${inputClass}_tarjeta`
			)[0].value;
			let tarjetaIds =
				document.getElementsByClassName(`inputRowsTarjeta`);
			let rowtarjetas = document.getElementsByClassName("rowtarjetas");
			allLength = true;
			...
			if (Tmonto.length > 0 && tarjeta.length > 0 && allLength) {
				...
				if (hasMore) {
					...
					for (tarjeta of inputRowsTarjeta) {
						...
						let withOutFormatMonto = parseFloat(
							monto.replace(",", "")
						).toFixed(2);
						totalGroup =
							totalGroup + parseFloat(withOutFormatMonto);
						//console.log(monto);
						let newTarjeta = {
							tipo: "tarjeta",
				...
				if (hasMore) {
					let btnTypeApartado =
						document.getElementById("btnTypeApartado");
					let abono =
						btnTypeApartado.classList.contains("active");
					let amounts =
						document.getElementById("totalFactAmount").dataset
							.amount;
					let montoTarjetaId =
						document.getElementById("montoTarjetaId").value;
					...
					_PAGA_CON +=
						_PAGA_CON.length > 0
							? `;Tarjeta,${parseFloat(totalGroup).toFixed(
									2
							  )}`
							: `Tarjeta,${parseFloat(totalGroup).toFixed(
									2
		...
		case "transferenciaInputs":
			let BnSelected = document.getElementsByClassName(
				`transferenciaInputs_banco`
			)[0].selectedIndex;
			let banco = document.getElementsByClassName(
				`transferenciaInputs_banco`
			)[0].options[BnSelected].text;
			let referencia = document.getElementsByClassName(
				`${inputClass}_referencia`
			)[0].value;
			let Trmonto = document.getElementsByClassName(
				`${inputClass}_monto`
			)[0].value;
			if (
				banco.length > 0 &&
...
function printFact(datos) {
	...
	let finalTexto = `<ul>${Texto}</ul><p><b>Total: <b>${total.toFixed(
		2
	)}</p>`;
	Swal.fire({
...
$("#bodyContent").on(
	"change",
	".fact_switchBtns input[type='checkbox']",
	function (e) {
...
$("#bodyContent").on("blur", ".cantInputFact", function (e) {
	if (this.value < 1) {
		...
		Swal.fire({
			...
			text:
				"La cantidad no puede ser 0 o una cantidad negativa, se cambia cantidad a 1 UND de. " +
				tr[1].innerText,
			icon: "error",
			confirmButtonText: "OK",
		});
...
$("#bodyContent").on("change", ".cantInputFact", function (e) {
	if (this.value < 1) {
		...
		Swal.fire({
			...
			title: "Error en la cantidad",
			text:
				"La cantidad no puede ser 0 o una cantidad negativa, se cambia cantidad a 1 UND. " +
				tr[1].innerText,
			icon: "error",
			confirmButtonText: "OK",
		});
	...
	if (resultCompare == true) {
		...
		if (resp.data !== false) {
				...
				if (jsonOferta) {
					...
					if (resutl) {
						...
						if (jsonOferta[id]["unica"] == 0) {
							...
							let aumentarMaxima =
								jsonOferta[id]["cantidadMaximaActual"];
							...
							if (jsonOferta[id]["productoOrlista"] == 1) {
								...
								let totalCodigos =
									jsonOferta[id]["productosID"]
										.length;
								//agrego a la cantidad maxima la cantidad por oferta X cantidad de productos
								jsonOferta[id]["cantidadMaximaActual"] =
									parseInt(aumentarMaxima) +
									parseInt(oferta.cantidad) *
										totalCodigos;
							} else if (
								jsonOferta[id]["productoOrlista"] == 2
							) {
								// solo por oferta
						...
						jsonOferta[`oferta_${oferta.idOferta}`] = {
							...oferta,
						};
						jsonOferta[`oferta_${oferta.idOferta}`][
							"productosID"
						...
						jsonOferta[`oferta_${oferta.idOferta}`][
							"cantidadMaximaActual"
						] =
							oferta.productoOrlista == 1
								? parseInt(oferta.cantidad) *
								  productosCodigo.length
								: oferta.cantidad;
					...
					jsonOferta[`oferta_${oferta.idOferta}`][
						...
						oferta.productoOrlista == 1
							? parseInt(oferta.cantidad) * totalCodigos
							: oferta.cantidad;
			...
			if (hasProductoOferta != undefined && hasProductoOferta.estado) {
				...
				data.descuento_descripcion =
					"Oferta id:" + hasProductoOferta.id;
			} else {
				hasProductoOferta = {
					estado: false,
					descuento: null,
			...
			let descuento =
				data.descuento == null ? "N/A" : data.descuento + "%";
			let des_descuento =
				data.descuento_descripcion == null
					? "Sin Descuento"
					: data.descuento_descripcion;
			let precios = calcTotalRowPrice(
				data.precio_venta,
				data.descuento == null ? 0 : data.descuento,
				data.activado_iva == 0 ? 0 : data.iva
			);
			let randnum = getRandom(10, 10000);

			let row = /*html*/ `
                        <tr class="productRowFac ${
						data.descuento == null ? "trNotDiscount" : ""
					}" id="itemRowProduct_${
				data.idproducto
			}_${randnum}"  data-hasoferta="${
				hasProductoOferta.estado ? 1 : 0
			}">
                            <td scope="row" data-oferta="${
							hasProductoOferta.estado ? 1 : 0
						}" data-codigo="${data.codigo}" data-id="${
				data.idproducto
			}" data-toggle="tooltip"data-placement="bottom" title="${
				data.idproducto
			}">${data.codigo}</td>
                            <td scope="row">${
							hasProductoOferta.estado
								? "(- OFERTA " + data.idOferta + "-)"
								: ""
						} ${data.descripcion_short} | ${data.marca}</td>
				...
				hasProductoOferta.estado ? 'max="1" disabled' : ""
			} class="cantInputFact" id="id_${
				data.idproducto
			}" style="width: 43px !important;text-align:center" name="" id="" value="1"  data-oferta="${
				oferta.idOferta
			}"></td>
							...
							data.descuento == null
								? 'class="tdNotDiscount"'
								: ""
						} data-toggle="tooltip" data-descuento="${
				data.descuento == null ? 0 : data.descuento
			}" data-placement="bottom" title="${des_descuento}">${descuento}</td>
                            <td scope="row" class="productRowFac_subtotal">${
							precios[0]
						}</td>
                            <td scope="row" class="productRowFac_total">${
							precios[1]
						}</td>
                            <td scope="row"><button class="btn btn-danger removeItemProductBtn" data-id="${
							data.idproducto
						}" data-rand="${randnum}" data-oferta="${
				oferta.idOferta == undefined && hasProductoOferta.estado
					? hasProductoOferta.id
					: oferta.idOferta
			}">X</button></td>
                        </tr>
            `;
	...
	if (jsonOferta) {
		...
		jsonOferta.id.map((e) => {
			...
			producto.map((cod) => {
				if (cod == codigo) {
					if (
						jsonOferta[id]["cantidadUsadaActual"] <
						jsonOferta[id]["cantidadMaximaActual"]
					) {
						jsonOferta[id]["cantidadUsadaActual"] =
							parseInt(
	...
	if (!deleteOferta) {
		...
		if (result) {
			...
			if (jsonOferta[id]["unica"] == 1) {
				//valido si la cantidad es por cada producto en la lista o solo por la oferta
				if (jsonOferta[id]["productoOrlista"] == 1) {
					// cantidad por cada producto en la oferta
					//consigo la canstidad de productos
					let totalCodigos =
						jsonOferta[id]["productosID"].length;
					//agrego a la cantidad maxima la cantidad por oferta X cantidad de productos
					jsonOferta[id]["cantidadMaximaActual"] =
						parseInt(jsonOferta[id].cantidad) * totalCodigos;
				} else if (jsonOferta[id]["productoOrlista"] == 2) {
					// solo por oferta
					//actualizo la cantidad maxima suamandole el valor de la cantidad de oferta
					jsonOferta[id]["cantidadMaximaActual"] = parseInt(
						jsonOferta[id].cantidad
					);
				}

				localStorage.setItem("ofertas", JSON.stringify(jsonOferta));
			} else {
				//si pueden repertirse la misma oferta
				//valido si la cantidad es por cada producto en la lista o solo por la oferta
				if (jsonOferta[id]["productoOrlista"] == 1) {
					//consigo la canstidad de productos
					let totalCodigos =
					...
					//agrego a la cantidad maxima la cantidad por oferta X cantidad de productos
					jsonOferta[id]["cantidadMaximaActual"] =
						parseInt(jsonOferta[id].cantidad) *
						totalCodigos *
						parseInt(cant);
				} else if (jsonOferta[id]["productoOrlista"] == 2) {
					// solo por oferta
		...
		if (row_delete.dataset.hasoferta == 1 && id != "oferta_undefined") {
			jsonOferta[id]["cantidadUsadaActual"] =
				parseInt(jsonOferta[id]["cantidadUsadaActual"]) - 1;
			localStorage.setItem("ofertas", JSON.stringify(jsonOferta));
			return;
		}
...
$("#bodyContent").on("click", ".removeItemProductBtn", function (e) {
	removeItemProduct(
		e.target.dataset.id,
		e.target.dataset.oferta,
		e.target.dataset.rand
	);
});
...
function calcTotalRowPrice(precio, descuento, iva) {
	...
	let precioVentaSub = parseFloat(
		precio - parseFloat(precio * descuentoPorcent)
	);
	let TotaliVa = 0;
	if (iva == 0) {
		...
		TotaliVa = parseFloat(
			precioVentaSub + parseFloat(precioVentaSub * ivaPorcent)
		);
	}
...
function RefreshCalcTotalRowPrice(tds, sum) {
	...
	let ivaPercent =
		parseInt($(tds[3]).text()) == 0 ? 1 : parseInt($(tds[3]).text());
	if (ivaPercent > 0) {
...
$("#bodyContent").on(
	"click",
	"#paginationModalCliente .paginationBtn",
	function (e) {
...
$("#bodyContent").on(
	"click",
	"#paginationModalCliente .pre_nexCliente",
	function (e) {
		let valueInput = document.getElementById("SearchClient_input").value;
		...
		.then((resp) => {
			...
			if (resp.error == "00000") {
				...
				if (!resp.data) {
					...
					let inputFacSaldoNow =
						document.getElementById("inputFacSaldoNow");
					let inputFacNewSaldoNow = document.getElementById(
						"inputFacNewSaldoNow"
					);
					inputFacSaldoNow.value = resp.data.Saldo;
					inputFacNewSaldoNow.value = parseFloat(
						resp.data.Saldo
					);
...
function SearchProductModalFact(toSearch, page) {
	...
	fetch("/facturacion/search/product/ctrlq", {
		...
		.then((resp) => {
			...
			let items = `<li class="page-item pre_nex ${
				resp.previouspage <= 0 ? "disabled" : ""
			}" data-minpage="1" data-page="${
				resp.previouspage <= 0 ? 1 : resp.previouspage
			}"><p class="page-link">Previous</p></li>`;
			for (let index = 0; index < paginacion.paginas; index++) {
				let pageNow = index + 1;
				items += `<li class="page-item paginationBtn ${
					pageNow == page ? "active" : ""
				}" data-page="${index + 1}"><a class="page-link" href="#">${
					index + 1
				}</a></li>`;
			}
			items += `<li class="page-item pre_nex ${
				resp.nextpage > paginacion.paginas ? "disabled" : ""
			}" data-maxpage="${paginacion.paginas}" data-page="${
				resp.nextpage >= paginacion.paginas
					? paginacion.paginas
					: resp.nextpage
			}"><p class="page-link">Next</p></li>`;

			resp.data.map((el, i) => {
				let url = el.image_url.split(",");
				let producto = `    <div class="col-6 ">
                                        <div class="card mb-3 codeToAddInputSearchCard" style="width: 100%;" data-codebar="${
									el.codigo
								}">
                                        <div class="row no-gutters">
                                            ...
                                            <img src="${
										url[0] == ""
											? "/public/assets/img/not-found.png"
											: url[0]
									}" class="card-img" alt="...">
                                            </div>
                                            <div class="col-md-8">
					...
					el.precio_venta
                                                ...
                                                <span class="icon_codeToAddInputSearch"><span class="codeToAddInputSearch">${
											el.codigo
										}</span></span>
                                                </br>
                                                <span class=""><small class="text-muted">${
											el.iva > 0
												? "<strong>IVA: </strong>" +
												  el.iva +
												  "%"
					...
					el.descuento > 0
						...
						: ""
                                                ...
                                                <span class=""><small class="text-muted">Marca: ${
											el.marca
										} | Categoria: ${
					el.categoria
				} | Talla: ${el.talla}</small></span>
                                                <p class=""><small class="text-muted">Estado: ${
											el.estado == 1
												? "Activo"
												: "Inactivo"
										}</small></p>
...
function searchClient(toSearch, page) {
	...
	fetch("/clientes/search/searchclient", {
		...
		.then((resp) => {
			$("#contentSearchClient").html("");
			let paginacion = resp.paginacion;
			let items = `<li class="page-item pre_nexCliente ${
				resp.previouspage <= 0 ? "disabled" : ""
			}" data-minpage="1" data-page="${
				resp.previouspage <= 0 ? 1 : resp.previouspage
			}"><p class="page-link">Previous</p></li>`;
			...
			items += `<li class="page-item pre_nexCliente ${
				resp.nextpage > paginacion.paginas ? "disabled" : ""
			}" data-maxpage="${paginacion.paginas}" data-page="${
				resp.nextpage >= paginacion.paginas
					? paginacion.paginas
...
function btnCerrarCajaEstado(id, monto) {
	...
	fetch("/facturacion/cajas/obtenerEstadoCajaEstado", {
		...
		.then((resp) => {
			...
			if (resp.error == "00000") {
				let data = resp.data;
				let btncerrarCajaFinal =
					document.getElementById("btncerrarCajaFinal");
				let total =
					resp.tarjeta +
					resp.efectivo +
					resp.transferencia +
					parseFloat(monto);
				let caja_total_facturado = document.getElementById(
					"caja_total_facturado"
				);
				caja_total_facturado.value =
					resp.tarjeta + resp.efectivo + resp.transferencia;
				let montos = `
                <div class="alert alert-primary d-flex justify-content-between" role="alert">
                <span>Caja Base:</span> <span>${monto}</span>
                </div>
                <div class="alert alert-success d-flex justify-content-between" role="alert">
                <span>Efectivo:</span> <span>${
					resp.efectivo.toFixed(2) == null
						? "0.00"
						: resp.efectivo.toFixed(2)
				}</span>
                </div>
                <div class="alert alert-success d-flex justify-content-between" role="alert">
                <span>Tarjetas:</span> <span>${
					resp.tarjeta.toFixed(2) == null
						? "0.00"
						: resp.tarjeta.toFixed(2)
				}</span>
                </div>
                <div class="alert alert-success d-flex justify-content-between" role="alert">
                <span>Transferencias:</span> <span>${
					resp.transferencia.toFixed(2) == null
						? "0.00"
						: resp.transferencia.toFixed(2)
				}</span>
                </div>
                <div class="alert alert-info d-flex justify-content-between" id="caja_totalsystem" data-total="${total.toFixed(
					2
				)}" role="alert">
                <span><strong>Total:</span> <span>${total.toFixed(
					2
				)}</strong></span>
                </div>
...
$("#bodyContent").on("blur", ".caja_blur", function (e) {
	...
	let totalsystem =
		document.getElementById("caja_totalsystem").dataset.total;
...
$("#bodyContent").on("change", "#apartadosSelect", function (e) {
	...
	btnMakeAbono.dataset.factura =
		apartado.options[apartado.selectedIndex].value;
	btnMakeAbono.dataset.abono =
		apartado.options[apartado.selectedIndex].dataset.abono;
	btnMakeAbono.dataset.total =
		apartado.options[apartado.selectedIndex].dataset.total;
	btnMakeAbono.dataset.saldo =
		apartado.options[apartado.selectedIndex].dataset.saldo;
	btnMakeAbono.dataset.fechaFinal =
		apartado.options[apartado.selectedIndex].dataset.fechaFinal;
	btnMakeAbono.dataset.fechaIncio =
		apartado.options[apartado.selectedIndex].dataset.fechaIncio;
});
	...
	fetch("/facturacion/apartados/getApartadosHasClient", {
		...
		.then((resp) => {
			if (resp.error == "00000") {
				...
				let apartadosSelect =
					document.getElementById("apartadosSelect");
				apartadosSelect.textContent = "";
				if (apartado !== []) {
					let opt = document.createElement("option");
					opt.appendChild(
						document.createTextNode(`Seleccione un apartado`)
					);
					opt.value = 0;
					opt.disabled = true;
					opt.selected = true;
				...
				apartado.map((item) => {
					let opt = document.createElement("option");
					let saldo =
						parseFloat(item.total) - parseFloat(item.abonado);
					opt.appendChild(
						document.createTextNode(
							`Fac# ${
								item.consecutivo
							} (Saldo: ${saldo.toFixed(2)}) (Total:${
								item.total
							})`
						)
					);
		...
		.then((resp) => {
			if (resp.error == "00000") {
				...
				products.map((item) => {
					...
					$("#productosListAbono").append(
						`<li  class="list-group-item">${item.descripcion_short.toUpperCase()} | <strong class="text-primary">Precio:</strong> ${
							item.precio
						} ${
							iva > 0
								? ` | <strong class="text-primary">IVA:</strong> ${item.iva}`
								: ""
						} | <strong class="text-primary">Cant:</strong> ${
							item.cantidad
...
function setAbono() {
	...
	let totalGroup = 0;
	let inputRowsTarjeta =
		document.getElementsByClassName("inputRowsTarjeta2");
	let stringCards = "";
	for (tarjeta of inputRowsTarjeta) {
		//console.log("Rows value", tarjeta.value);
		let montoId = `tarjetaMonto_${tarjeta.dataset.id}`;
		let monto = document.getElementById(montoId).value;
		let withOutFormatMonto = parseFloat(monto.replace(",", "")).toFixed(
			2
		);
...
$("#bodyContent").on("click", "#addNewTarjeta2", function (e) {
	...
	input2.classList.add(
		"form-control",
		"tarjertaInputs_monto2",
		"inputRowsTarjetaMonto2",
		"lbMontoToPay"
	);
	input2.value = "0.00";
	input2.id = "tarjetaMonto_" + randID;
