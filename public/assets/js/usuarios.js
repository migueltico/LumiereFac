...
$("#bodyContent").on("click", "#usuarios_add_btnAddUsuarios", function (e) {
	let formData = new FormData(
		document.getElementById("usuarios_AddUsuariosForm")
	);
...
$("#bodyContent").on("click", ".btnDisableUser", function (e) {
	//get data-id from button
	let id = e.target.dataset.id;
	let formData = new FormData();
	formData.append("id", id);
	fetch("/usuarios/disableUser", {
		method: "POST",
		body: formData,
	})
		.then((resp) => resp.json())
		.then((resp) => {
			...
			if (resp.error == "00000") {
				Swal.fire({
					position: "top",
					title: "Se desactivo el usuario correctamente",
					icon: "success",
					confirmButtonText: "OK",
					timer: 2500,
				});
	...
	fetch("/usuarios/getUserById", {
		...
		.then((resp) => {
			...
			if (resp.error == "00000") {
				...
				let usuarios_edit_nombre = document.getElementById(
					"usuarios_edit_nombre"
				);
				let usuarios_edit_correo = document.getElementById(
					"usuarios_edit_correo"
				);
				let usuarios_edit_telefono = document.getElementById(
					"usuarios_edit_telefono"
				);
				let usuarios_edit_direccion = document.getElementById(
					"usuarios_edit_direccion"
				);
				let usuarios_edit_rol =
					document.getElementById("usuarios_edit_rol");
				let data = resp.data;
...
function loadDataEditModelUsersPerfil(el, formId) {
	...
	fetch("/usuarios/getUserById", {
		...
		.then((resp) => {
			//console.log(resp);
			if (resp.error == "00000") {
				let perfil_edit_nombre =
					document.getElementById("perfil_edit_nombre");
				let perfil_edit_correo =
					document.getElementById("perfil_edit_correo");
				let perfil_edit_telefono = document.getElementById(
					"perfil_edit_telefono"
				);
				let perfil_edit_direccion = document.getElementById(
					"perfil_edit_direccion"
				);
				let data = resp.data;
...
$("#bodyContent").on("click", "#usuarios_edit_btneditUsuarios", function (e) {
	let formData = new FormData(
		document.getElementById("usuarios_editUsuariosForm")
	);
	fetch("/usuarios/editUser", {
		...
		.then((resp) => {
			...
			if (resp.error == "00000") {
				document
					.getElementById("usuarios_editUsuariosForm")
					.reset();
				Swal.fire(
					{
						position: "top",
...
$("#bodyHtml").on("click", "#perfil_edit_btneditperfil", function (e) {
	let formData = new FormData(
		document.getElementById("perfil_editperfilForm")
	);
		...
		.then((resp) => {
			...
			if (resp.error == "00000") {
				if (!resp.newuser && !resp.same) {
					let perfil_edit_usuario = document.getElementById(
						"perfil_edit_usuario"
					);
					perfil_edit_usuario.value = resp.olduser;
					perfil_edit_usuario.classList.add(
						"border",
						"border-danger"
					);
					Swal.fire({
					...
					perfil_edit_usuario.classList.remove(
						"border",
						"border-danger"
					);
				}
...
$("#bodyHtml").on("click", "#perfil_update_pass", async function (e) {
	...
	let perfil_edit_pass_now = document.getElementById(
		"perfil_edit_pass_now"
	).value;
	let perfil_edit_new_pass = document.getElementById(
		"perfil_edit_new_pass"
	).value;
	let perfil_edit_confirm_pass = document.getElementById(
		"perfil_edit_confirm_pass"
	).value;
	let confirm = await confirmPassNow();
	let samePass =
		perfil_edit_new_pass.trim() == perfil_edit_confirm_pass.trim() &&
		perfil_edit_new_pass.trim() != "" &&
		perfil_edit_confirm_pass.trim() != ""
			? true
			: false;
	if (!samePass) {
		Swal.fire({
