...
class facturacionModel
{
    ...
    public static function getHistorialDiario($start = null, $end = null)
    {
        ...
        $sql  = "SELECT f.*, DATE_FORMAT(fecha,'%d-%m-%Y') fechaFormat, c.nombre cliente FROM facturas f LEFT JOIN cliente c on c.idcliente = f.idcliente WHERE formatDate BETWEEN '$start' AND '$end' AND idusuario = $iduser AND estado <> 2 ORDER BY consecutivo DESC";
        $sql1 = "SELECT f.*, DATE_FORMAT(fecha,'%d-%m-%Y') fechaFormat, c.nombre cliente FROM facturas f LEFT JOIN cliente c on c.idcliente = f.idcliente WHERE formatDate BETWEEN '$start' AND '$end' AND estado <> 2 ORDER BY consecutivo DESC";
        $sql2 = "SELECT f.*, DATE_FORMAT(fecha,'%d-%m-%Y') fechaFormat, c.nombre cliente FROM facturas f LEFT JOIN cliente c on c.idcliente = f.idcliente WHERE formatDate BETWEEN '$start' AND '$end' AND idusuario = $iduser AND estado <> 2 ORDER BY consecutivo DESC";
        if ($pos === false) {
            $sql = $sql2;
        } else {
            $sql = $sql1; //admin
